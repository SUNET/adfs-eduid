...
using ADFSTK.ExternalMFA.Common.Utils;
...
namespace ADFSTK.ExternalMFA
{
    public class ExternalRefedsMFAAdapter : IAuthenticationAdapter
    {
        ...
        {
            return new ExternalRefedsPresentation(username, proxyUrl,trustedUrl);
        }
        ...
        {
            return new ExternalRefedsPresentation(username, secret, ex);
        }
        ...
        public IAdapterPresentation BeginAuthentication(Claim identityClaim, HttpListenerRequest request, IAuthenticationContext authContext)
        {
            ...
            Log.WriteEntry("Getting logipage: ", EventLogEntryType.Information, 335);
            ...
            Log.WriteEntry("Saving to context ", EventLogEntryType.Information, 335);
            ...
            Log.WriteEntry("encrypted object: ", EventLogEntryType.Information, 335);
            ...
            Log.WriteEntry("Getting trustedurl from: " + _externalMFASettings.ProxySp + ", Got: " + trustedUrl, EventLogEntryType.Information, 335);
            Log.WriteEntry("Getting loginpage: ", EventLogEntryType.Information, 335);
            ...
            if (configData != null)
            {
                if (configData.Data != null)
                {
                    using (StreamReader reader = new StreamReader(configData.Data, Encoding.UTF8))
                    {
                        ...
                        {
                            Log.WriteEntry("Unable to load ExternalMFA config data. Check that it is registered and correct." + ex.Message + ex.StackTrace, EventLogEntryType.Information, 335);
                    ...
                    try
                    {
                        ...
                        {
                            Log.WriteEntry("No Settings provided for ExternalIdpMFAAdapter", EventLogEntryType.Error, 335);
                        }
                    }
                    catch (Exception ex)
                    {
                        Log.WriteEntry("Unable to configure ExternalIdpMFAAdapter " + ex.Message + ex.StackTrace, EventLogEntryType.Error, 335);
                        throw new ArgumentException();
                    }
                }
            }
        ...
        {
            Log.WriteEntry("Entering ExternalIdpMFAAdapter TryEndAuthentication", EventLogEntryType.Information, 335);
            if (null == authContext)
            {
            ...
            {
                Log.WriteEntry("ExternalIdpMFAAdapter identity not present ", EventLogEntryType.Error, 335);
            ...
            {
                Log.WriteEntry("ExternalIdpMFAAdapter, Token not present ", EventLogEntryType.Error, 335);
            ...
            Log.WriteEntry("TryEndAuthentication View: " + view, EventLogEntryType.Information, 335);
            if (view == Constants.ResourceNames.ViewStart)
            {
                //Get token;
                string token = (string)authContext.Data[Constants.AuthContextKeys.Token];
                Log.WriteEntry("Got token: " + token, EventLogEntryType.Information, 335);
                ...
                Log.WriteEntry("Decrypted token: " + urldecodedEncString, EventLogEntryType.Information, 335);
                var authResponse = JsonConvert.DeserializeObject<AuthRequest>(urldecodedEncString);
                Log.WriteEntry("Got uniqueidentifier from proxy: " + authResponse.IdentityClaimValue, EventLogEntryType.Information, 335);
                
                //Check valid time, valid user etc
                if(authResponse!=null)
                {
                    Log.WriteEntry("Authreq not null: ", EventLogEntryType.Information, 335);
                    //valid time?
                    if (authResponse.AuthInstant < DateTime.UtcNow.AddMinutes(5))
                    {
                        Log.WriteEntry("Get unique identifier from personservice: ", EventLogEntryType.Information, 335);
                        var ssn = _personService.GetUniqueIdentifier((string)authContext.Data[Constants.AuthContextKeys.Identity]);
                        Log.WriteEntry("Got unique identifier from personservice: ", EventLogEntryType.Information, 335);
                        //valid token and ssn
                        if (token == authResponse.Token && _personService.UniqueIdentifierValid(ssn, authResponse.IdentityClaimValue))
                        {
                            Log.WriteEntry("token valid, unique identifier valid: ", EventLogEntryType.Information, 335);
